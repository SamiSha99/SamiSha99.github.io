...
{
    let starinnerHTML = "";
    let filled = Math.floor(rating), empty = 5 - Math.floor(rating);
    let half = (rating % 1 != 0);
    
    if(half) empty--;

    for(f = 0; f < filled; f++) starinnerHTML += '<i class="fas fa-star"></i>';
    if(half) starinnerHTML += '<i class="fas fa-star-half-alt"></i>';
    for(e = 0; e < empty; e++) starinnerHTML += '<i class="far fa-star"></i>';
